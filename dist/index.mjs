...
var currentExtVersion = "4.0.115";
var currentUIVersion = "1.0.124";
var currentIconsVersion = "1.0.58";
...
async function getLatestIconVersion() {
  try {
...
async function getLatestExtVersion() {
...
async function getProjectState(targetPath) {
  let localUIVer = null;
  let hasPackageJson = false;
  let localIconsVer = null;
...
async function showSmartMenu(targetPath, isPremium, state) {
  const choices = [];
  ...
  if (!state.localUIVer) {
    choices.push({ name: `${chalk.bold("Install MI\xD0GAR\xD0R UI")} ${chalk.dim(`(v${state.remoteUIVer})`)}`, value: isPremium ? "full-premium-install" : "full-install" });
    ...
    choices.push({ name: `${chalk.yellow("Update")} MI\xD0GAR\xD0R UI Components ${chalk.dim(`(${state.localUIVer} \u2192 ${state.remoteUIVer})`)}`, value: "ui-components-and-libs" });
    choices.push({ name: `${chalk.bold("Interactive UI Menu")} ${chalk.gray("\u2192 Components + Libraries + CSS")}`, value: "ui-interactive" });
    choices.push({ name: `${chalk.bold("Configure Only")} ${chalk.gray("\u2192 Tailwind + PostCSS setup")}`, value: "ui-configure-tailwind-postcss" });
    choices.push({ name: `${chalk.bold("Select Components")} ${chalk.gray("\u2192 Specific free components")}`, value: "ui-select-components" });
    choices.push({ name: `${chalk.bold("Configure with Ngin")} ${chalk.gray("\u2192 Tailwind + PostCSS + Ngin preset")}`, value: "ui-configure-ngin" });
  ...
  if (!state.localIconsVer) {
    choices.push({ name: "Add MI\xD0GAR\xD0R Icons", value: "install-icons" });
  } else if (state.localIconsVer !== state.remoteIconsVer) {
    choices.push({ name: `Update MI\xD0GAR\xD0R Icons ${chalk.dim(`(${state.localIconsVer} \u2192 ${state.remoteIconsVer})`)}`, value: "update-icons" });
  }
...
MI\xD0GAR\xD0R UI ${isPremium ? "- PREMIUM" : ""}`));
...
async function CreateRemix2() {
  ...
  spawnSync(["npx", "create-remix@latest", projectName], {
    stdio: ["inherit", "inherit", "inherit"]
  });
  return projectName;
}
async function handleAction(action, isPremium, targetPath, state) {
  switch (action) {
    case "create-new":
      const projectFolderName = await CreateRemix2();
      if (projectFolderName) {
        const projectPath = path.join(targetPath, projectFolderName);
